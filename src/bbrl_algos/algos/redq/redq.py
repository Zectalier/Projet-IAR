...
def run_redq(cfg, logger, trial=None):
    ...
    if cfg.collect_stats:
        ...
        # Count the number of files with redq-steps-*.data in the directory
        run_number = len([f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and "redq-steps-" in f])

        filename_steps = directory + "redq-steps-" + str(run_number) + "-" + cfg.gym_env.env_name + ".data"
