...
# %%
def create_dqn_replay_buffer_agent(cfg_algo, train_env_agent, eval_env_agent):
...
# %%
def run_dqn_replay_buffer(cfg, logger, trial=None):
    best_reward = float("-inf")
    best_agent = None

    if cfg.collect_stats:
        directory = "./dqn_replay_buffer_data/"
        if not os.path.exists(directory):
            os.makedirs(directory)
        filename = directory + "dqn_replay_buffer_" + cfg.gym_env.env_name + ".data"
        fo = open(filename, "wb")
        stats_data = []
    ...
    # 2) Create the dqn_replay_buffer-like Agent
    train_agent, eval_agent, q_agent = create_dqn_replay_buffer_agent(
...
@hydra.main(
    config_path="configs/",
    # config_name="dqn_replay_buffer_cartpole.yaml",
    config_name="dqn_replay_buffer_lunar_lander.yaml",
)  # , version_base="1.3")
def main(cfg_raw: DictConfig):
    torch.random.manual_seed(seed=cfg_raw.algorithm.seed.torch)

    if "optuna" in cfg_raw:
        launch_optuna(cfg_raw, run_dqn_replay_buffer)
    else:
        logger = Logger(cfg_raw)
        run_dqn_replay_buffer(cfg_raw, logger)
