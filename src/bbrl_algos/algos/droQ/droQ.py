...
from droQ_critic import ContinuousQAgent
...
def create_droq_agent(cfg, train_env_agent, eval_env_agent, M):
    obs_size, act_size = train_env_agent.get_obs_and_actions_sizes()
    
    assert (
        train_env_agent.is_continuous_action()
    ), "droq code dedicated to continuous actions"
...
def compute_critic_loss(
    ...
    min_tensor = post_q_values[0]
    for tensor in post_q_values[1:]:
        min_tensor = torch.min(min_tensor, tensor)

    q_next = min_tensor.squeeze(-1)
...
def run_droq(cfg, logger, trial=None):
    ...
    (
        ...
        critics,
        target_critics,
    ) = create_droq_agent(cfg, train_env_agent, eval_env_agent, M)
    ...
    while nb_steps < cfg.algorithm.n_steps:
        ...
        if nb_steps - tmp_steps > cfg.algorithm.eval_interval:
            ...
            if cfg.save_best and best_reward == mean:
                save_best(
                    actor, cfg.gym_env.env_name, mean, "./droq_best_agents/", "droq"
                )
    ...
    if cfg.collect_stats:
        ...
        run_number = len([f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and "droq-" in f])
        filename = directory + "droq-" + str(run_number) + "-" + cfg.gym_env.env_name + ".data"
